...
const MODE_CONFIG: {
  ...
  },
  {
    ...
    title: "Make targeted edits to files",
  },
  {
    mode: "agent",
    label: "Agent",
...
export function ChatPanel() {
  const chatMessages = useProjectStore((s) => s.config.chatMessages);
  ...
  const buildSnapshot = (): AgentWorkspaceSnapshot => {
    ...
    const files = collectFiles(snap.config.tree);
    return {
      chapters: files.map((f, i) => {
        const entry = snap.openFiles.get(f.path);
        return {
          id: f.path,
          title: f.name,
          order: i,
          plainText: entry ? jsonToPlainText(entry.content) : "",
        };
      }),
      researchDocuments: [],

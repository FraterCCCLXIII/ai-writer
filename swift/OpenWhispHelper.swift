...
func listenForFnKey() -> Int32 {
    guard inputMonitoringGranted() else {
        emitJSON(EventMessage(type: "error", message: "Input Monitoring is not enabled for Manuscript."))
        return 1
    }
    ...
    guard let tap = CGEvent.tapCreate(
        ...
        emitJSON(EventMessage(type: "error", message: "Manuscript could not create the global Fn listener."))
        return 1
    }
